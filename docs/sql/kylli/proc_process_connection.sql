...
BEGIN
    ...
    IF @ksugu1 = '' AND @ssugu1 != '' THEN
        UPDATE kirjed SET sugu = @ssugu1, user = _user WHERE isikukood = @ik1;
    END IF;
    IF @ksugu2 = '' AND @ssugu2 != '' THEN
        UPDATE kirjed SET sugu = @ssugu2, user = _user WHERE isikukood = @ik2;
    END IF;
    IF @ksugu1 != '' AND @ssugu1_1 = '=' THEN
        UPDATE kirjed SET sugu = @ksugu1, user = _user WHERE isikukood = @ik2;
    END IF;
    IF @ksugu2 != '' AND @ssugu1_1 = '=' THEN
        UPDATE kirjed SET sugu = @ksugu2, user = _user WHERE isikukood = @ik1;
    END IF;
    IF @ksugu1 = 'M' AND @ssugu1_1 = 'X' THEN
        UPDATE kirjed SET sugu = 'N', user = _user WHERE isikukood = @ik2;
    END IF;
    IF @ksugu1 = 'N' AND @ssugu1_1 = 'X' THEN
        UPDATE kirjed SET sugu = 'M', user = _user WHERE isikukood = @ik2;
    END IF;
END;;

CREATE OR REPLACE DEFINER=`queue`@`localhost` PROCEDURE process_connection(IN _id INT(11) UNSIGNED, IN _user VARCHAR(50))
BEGIN
    DECLARE msg VARCHAR(200);

    CALL set_sex_from_connection(_id, _user);
    ...
    IF @sl2 IS NULL THEN
        IF     @ssugu1_1 = 'M' OR @ksugu2 = 'M' THEN
            UPDATE seosed SET vastasseos = @sl1M, user = _user WHERE id = _id;
        ELSEIF @ssugu1_1 = 'N' OR @ksugu2 = 'N' THEN
            UPDATE seosed SET vastasseos = @sl1N, user = _user WHERE id = _id;
        ...
        ELSEIF @ssugu1_1 IS NULL THEN
            ...
            ELSEIF @ksugu2 = 'N' THEN
                UPDATE seosed SET vastasseos = @sl1N, user = _user WHERE id = _id;
            ELSEIF @ksugu2 = '' THEN
                UPDATE seosed SET vastasseos = @sl1X, user = _user WHERE id = _id;
            END IF;
        END IF;
    END IF;
    ...
    IF @sl1 IS NULL THEN
        IF     @ssugu2_1 = 'M' OR @ksugu1 = 'M' THEN
            UPDATE seosed SET seos = @sl2M, user = _user WHERE id = _id;
        ELSEIF @ssugu2_1 = 'N' OR @ksugu1 = 'N' THEN
            UPDATE seosed SET seos = @sl2N, user = _user WHERE id = _id;
        ...
        ELSEIF @ssugu2_1 IS NULL THEN
            ...
            ELSEIF @ksugu1 = 'N' THEN
                UPDATE seosed SET seos = @sl2N, user = _user WHERE id = _id;
            ELSEIF @ksugu1 = '' THEN
                UPDATE seosed SET seos = @sl2X, user = _user WHERE id = _id;
            END IF;
        END IF;
    END IF;

...
DELIMITER ;; -- desktop_BU
  ...
  proc_label:BEGIN
    ...
    IF  OLD.created_by != user() AND user() != 'event_scheduler@localhost' THEN -- can change only owned records
      SELECT concat_ws('\n'
    ...
    IF NEW.persoon != OLD.persoon THEN
      IF NEW.persoon = '' THEN
        ...
        SIGNAL SQLSTATE '03100' SET MESSAGE_TEXT = msg;
      END IF;
      IF OLD.kirjekood = OLD.persoon THEN
        SELECT concat_ws('\n'
      ...
      IF OLD.persoon != '' THEN
        INSERT IGNORE INTO repis.z_queue (kirjekood1,  kirjekood2, task,            params, created_by)
        VALUES                           (OLD.persoon, NULL,       'desktop_NK_refresh', NULL,   user());
      END IF;

      INSERT IGNORE INTO repis.z_queue (kirjekood1,  kirjekood2, task,            params, created_by)
      VALUES                           (NEW.persoon, NULL,       'desktop_NK_refresh', NULL,   user());

    END IF;

    IF OLD.allikas in ('EMI', 'TS', 'Nimekujud') THEN
    ...
    ELSEIF NEW.valmis = 'Valmis' THEN
      ...
      SET k.persoon = d.persoon,
          ...
          k.updated_at = now(), updated_by = user();
...
DELIMITER ;; -- desktop_NK_refresh
  ...
  proc_label:BEGIN
    ...
    THEN
        SELECT 'Nimekuju saab arvutada ainult NK kirjele!' INTO msg;
        SIGNAL SQLSTATE '03100' SET MESSAGE_TEXT = msg;
    END IF;
